...
void BouncingBox::draw() 
{
	...
	// Use code below for bounding sphere drawing

	//glBegin(GL_LINE_LOOP);
	//double PI =8*atan(1);
	//double angleVal = 2*PI/360; 
	//for(register int i = 0; i<360 ;++i)
	//{
	//	glVertex2d(		(cos((angleVal)*i)*m_boundingSphereRadius) + m_position.x,
	//					(sin((angleVal)*i)*m_boundingSphereRadius)+ m_position.y);    
	//}	
	//glEnd(); 
